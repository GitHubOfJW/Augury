layui.define(['form','laydate', 'table', 'global'], function(exports) {
    ...
    global.ajaxSetup($);
      ...
      $('button[lay-filter="add"]').click(function(){
        global.dialog_show('添加礼品','/pages/gift/add/',null,null)
      })
      ...
      if(user && user.is_admin){
        ...
        //列表
      const cols =  [[ //表头
          {type:'checkbox',title:'选择',fixed:'left',width:'60',sort: true}
          ,{field: 'id', title: 'ID', sort: true, width:'100',sort: true}
          ,{field: 'name', title: '礼品名称', width:'140',sort: true}//
        ...
        const url = global.absoluteUrl('/api/v1/gift/list')
        ...
        global.on("giftListRefresh",(isEdit)=>{
        ...
        table.on('tool(giftList)', function(obj){
          switch(obj.event){
            ...
            break;
            case 'edit':
              global.dialog_show('编辑',`/pages/gift/edit/#/gift_id=${obj.data.id}`)
            break;
          };
        });
