layui.define(['form', 'global'], function(exports) {
   const form = layui.form,
   $ = layui.jquery,
   global = layui.global
    
    //自定义验证规则
    form.verify({
        name: function(value){
        if(value.length < 3){
            return '名称至少得2个字符啊';
        }
        }
        ,pass: [/(.+){6,16}$/, '密码必须6到16位']
        ,repass: function(value){
            if($('#password').val()!=$('#repass').val()){
                return '两次密码不一致';
            }
        }
    });
    
   const isDialog = !!parent.layer.getFrameIndex(window.name)
  let body = {};
  if(isDialog && parent.$){
    const iframe =  parent.$(`iframe[name=${window.name}]`)
    body = global.getParamsWithUrl(iframe.attr('src'))
  }
  


  // 获取所有的角色
  let roleId = null
  global.ajax({
    url:'/api/v1/roleAll/list',
    type: 'GET',
    success:function(data){
        console.log(data)
        const html = template('roleList',{roles:data.data.list})
        $('.roleList').html(html)
        if(roleId){
            $('#role'+roleId).prop('checked',true)
        }
        form.render('radio')
    }
  })

   if(body.admin_id){
       global.ajax({
           url:'/api/v1/admin/detail/'+body.admin_id,
           type:'GET',
           success:function(data){
               if(!data.code){
                 $('#account').val(data.data.account)
                 $('#mobile').val(data.data.mobile)
                 $('#name').val(data.data.name)
                 $('#jst_account').val(data.data.jst_account)
                 $('#wechat').val(data.data.wechat)
                 $('#wechat_name').val(data.data.wechat_name)
                 $('#qq').val(data.data.qq)
                 $('#qq_name').val(data.data.qq_name)
                 $('#computer').val(data.data.computer)
                 $('#enable'+(data.data.enable?'1':'0')).prop('checked',true)

                 if(data.data.is_admin){
                    $('.admin-enable').css('display','none')
                    $('.roleItem').css('display','none')
                 }

                 roleId = data.data.role_id
                 $('#role'+data.data.role_id).prop('checked',true)
                 form.render('radio')
               }
           },
           err:function(data){
 
           }
       })
   }
   

  // 监听密码修改
  $('#password').on('input',function(){
      if($(this).val() == '----------'){
        $(this).removeAttr('name')
        $('.password-default').css('display','none')
      }else{
        $(this).attr('name','password')
        $('.password-default').css('display','block')
      }
  })
  // 监听恢复默认
  $('.password-default').css('display','none')
  $('.password-default').click(function(){
    $('#password').removeAttr('name')
    $('#password').val('----------')
    $(this).css('display','none')
  })

  
  //监听提交
  form.on('submit(edit)', function(data){
    // 弹出提示
    let index = null;
    const id = setTimeout(() => {
        clearTimeout(id);
        if(!index){
            index = layer.msg('玩命卖萌中...',{type:1,shade:0.5,time:0});
        }
    }, 1000);

    global.ajax({
        url:'/api/v1/admin/edit/'+body.admin_id,
        traditional:true,
        data:data.field,
        method:'POST',
        type:'json',
        success:function(data){
            layer.close(index);
            index = " ";
            //发异步，把数据提交给php
            if(!!data.code){
                layer.msg(data.message,{icon:2,shade:0.2,time:500});
                return;
            }

            layer.alert(data.message, {icon: 6},function () {
                global.emit("adminListRefresh",true)
                // 获得frame索引
                var index = parent.layer.getFrameIndex(window.name);
                //关闭当前frame
                parent.layer.close(index);
            });
        },
        error:function(err){
            layer.msg('服务器开小差了',{icon:2,shade:0.2,time:500});
            layer.close(index);
            index = " ";
        }
    })
    
    return false;
    });

   exports('adminEdit',{})
});