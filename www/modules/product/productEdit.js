layui.define(['form', 'global'], function(exports) {
  ...
  global.ajax({
      ...
      success:function(data){
         if(!data.code){
            ...
            if(cooperate_status){
                $('select[name="cooperate_status"]').val(cooperate_status)
            }
            form.render('select')
         }
      },
  ...
  if(body.product_id){
      global.ajax({
          url:'/api/v1/product/detail/'+body.product_id,
          type:'GET',
          success:function(data){
              if(!data.code){
                ...
                $('#origin_price').val(data.data.origin_price)
                $('#commission').val(data.data.commission)
                $('#service_price').val(data.data.service_price)
                cooperate_status = data.data.cooperate_status;
                $('select[name="cooperate_status"]').val(data.data.cooperate_status)
  ...
  form.on('submit(edit)', function(data){
    ...
    global.ajax({
        url:'/api/v1/product/edit/'+body.product_id,
        traditional:true,
        data:data.field,
        method:'POST',
        type:'json',
        success:function(data){
            ...
            layer.alert(data.message, {icon: 6},function () {
                global.emit("productListRefresh",true);
                // 获得frame索引
