...
if(body.consumer_id){
    global.ajax({
        url:'/api/v1/consumer/detail/'+body.consumer_id,
        type:'GET',
        success:function(data){
            if(!data.code){
              $('#num_id').val(data.data.num_id)
              $('#name').val(data.data.name)
              $('#mobile').val(data.data.mobile)
              $('#wechat').val(data.data.wechat)
              $('select[name="wechat_mobile_match"]').val(data.data.wechat_mobile_match)
              $('select[name="gender"]').val(data.data.gender)
              $('#recommand').val(data.data.recommand)
  ...
  form.on('submit(edit)', function(data){
    ...
    global.ajax({
        url:'/api/v1/consumer/edit/'+ body.consumer_id,
        traditional:true,
        data:data.field,
        method:'POST',
        type:'json',
        success:function(data){
            ...
            layer.alert(data.message, {icon: 6},function () {
                global.emit("consumerListRefresh",true)
                // 获得frame索引
