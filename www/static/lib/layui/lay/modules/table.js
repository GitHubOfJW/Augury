...
function(e) {
    ...
    c = function() {
        ...
        t = e.config,
        i = t.id || t.index;
        return i && (c.that[i] = e, c.config[i] = t),obj = 
        {
            reload: (t)=>{
                e.reload.call(e, t)
                // 更新config，用于外部获取数据参数
    ...
    F.prototype.renderToolbar = function() {
        ...
        function(e, t) {
            var i = r[t];
            i && d.push('<div class="layui-inline" style="width:80px;color:#222288;" title="' + i.title + '" lay-event="' + i.layEvent + '">'+i.title+'</div>') //<i class="layui-icon ' + i.icon + '"></i>
        }),
        ...
        m = function() {
            ...
            function(a, l) {
                ...
                0 !== l.length && (r || (l[d.config.indexName] = a), c.eachCols(function(n, r) {
                    ...
                    // 主动解析keyPath zjw 让table 支持keyPath 解析
                    if(c && typeof c  == 'string' && c.split('.').length > 1){
                      const keyPaths = c.split('.');
                      let temp  = l
                      for(let j = 0;j < keyPaths.length ; j ++){
                         temp = temp[keyPaths[j]];
                         if(!temp){
                           if(r.templet){
                            temp = r.templet(l);
                           }else{
                      ...
                      p = temp;
                      l[c] = temp;// 解析完毕后将值赋值给本来的数据
                    }
                    
                    if (void 0 !== p && null !== p || (p = ""), !r.colGroup) {
                        let cellstyle = '';
                        if(s.cellstyle && !r.toolbar){
                            cellstyle =  s.cellstyle(l,r) || ""
                        }
                        var v = ['<td style="'+cellstyle+'" data-field="' + c + '" data-key="' + h + '" ' +
                        function() {
                            var e = [];
            ...
            case "LAYTABLE_EXPORT":
                r.ie ? l.tips("导出功能不支持 IE，请用 Chrome 等高级浏览器导出", this, {
                    ...
                    list: function() {
                        ...
                        return ['<li data-type="xls">导出到 Excel 文件</li>'].join("")
                    } (),
                    done: function(e, i) {
                        i.on("click",
                        function() {
    ...
    d.exportFile = function(e, t, i) {
        ...
        // 远程导出
        if(a.export && typeof a.export == 'function'){
          // 导出的数据
          a.export();
